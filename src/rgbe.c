...
float2rgbe(unsigned char rgbe[4], float red, float green, float blue)
{
  ...
  if (v < 1e-32) {
	...
	// Changed to frexpf from frexp to get rid of compilation warning double -> float
...
rgbe2float(float *red, float *green, float *blue, unsigned char rgbe[4])
{
  ...
  if (rgbe[3]) {   /*nonzero pixel*/
	// Changed to ldexpf from ldexp to get rid of compilation warning double -> float
    f = ldexpf(1.0,rgbe[3]-(int)(128+8));
    *red = rgbe[0] * f;
    *green = rgbe[1] * f;
    *blue = rgbe[2] * f;
  }
